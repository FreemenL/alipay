...
async function queryResult(){
  ...
  let bizContent = {
    out_trade_no: out_trade_no, // 根据时间戳来生成一个订单号,
  }
  let result = await alipay.exec(method, { bizContent });
...
async function createOrder(goods) {
    ...
    let bizContent = {
      ...
      notify_url: "http://123.57.241.11/description"
    }
    ...
    let result = await alipay.exec(method, { bizContent });
    console.log('out_trade_no :>> ', out_trade_no);
    let qrcode = await QRCode.toDataURL(result.qrCode);
    result={
      ...result,
      qrcode
    }
